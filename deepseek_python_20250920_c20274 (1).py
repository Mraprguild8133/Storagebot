...
# -----------------------------
async def download_with_progress(client, message, file_id, file_name, file_size, user_id):
    """Turbo download file from Telegram with progress tracking"""
    progress_id = f"download_{user_id}_{message.id}"
...
async def upload_file_handler(client, message: Message):
    ...
    # FIX: Handle cases where message.from_user is None (channels, anonymous users)
    if message.from_user:
        user_id = message.from_user.id
        user_name = message.from_user.first_name or "User"
    elif message.sender_chat:
        user_id = message.sender_chat.id
        user_name = message.sender_chat.title or "Channel"
    else:
        user_id = 0  # Fallback for anonymous users
        user_name = "Anonymous"
    
    # Get user ID for folder naming - use chat ID as fallback
    effective_user_id = user_id if user_id != 0 else message.chat.id
    ...
    try:
        # Download with progress - pass user_id explicitly
        download_start = time.time()
        file_path = await download_with_progress(client, status_message, media.file_id, file_name, size, effective_user_id)
        download_time = time.time() - download_start
        
        # Update message for upload phase
        await status_message.edit_text("⚡ Starting turbo upload...")
        
        user_file_name = f"{get_user_folder(effective_user_id)}/{file_name}"
        upload_id = f"upload_{effective_user_id}_{status_message.id}"
...
# -----------------------------
# Additional Commands with user validation
# -----------------------------
