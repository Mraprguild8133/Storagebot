...
class DownloadProgress:
    """Class to track download progress for parallel downloads"""
    def __init__(self, file_size, total_chunks, status_message):
    ...
    async def update(self, chunk_size):
        ...
        async with self.lock:
            ...
            if current_time - self.last_update_time >= 1:
                percentage = (self.downloaded_bytes / self.file_size) * 100
                elapsed_time = current_time - self.start_time
                
                # Calculate speed with smoothing
                speed = (self.downloaded_bytes - self.last_downloaded_bytes) / (current_time - self.last_update_time)
                
                # Calculate ETA
                if speed > 0:
                    eta = (self.file_size - self.downloaded_bytes) / speed
                else:
                    eta = 0
                ...
                progress_text = (
                    ...
                    f"Processed: {humanbytes(self.downloaded_bytes)} of {humanbytes(self.file_size)}\n"
...
async def download_in_parallel_chunks(client, message, file_id, file_size, temp_file, status_message):
    ...
    progress_tracker = DownloadProgress(file_size, total_chunks, status_message)
    ...
    for chunk_index in range(total_chunks):
        task = asyncio.create_task(
            download_chunk(
                ...
                chunk_index, file_size, semaphore, progress_tracker
            )
        )
        tasks.append(task)
...
async def download_chunk(client, message, file_id, temp_file, chunk_index, file_size, semaphore, progress_tracker):
    ...
    async with semaphore:
        ...
        for attempt in range(3):
            try:
                ...
                # Update progress
                await progress_tracker.update(chunk_size)
                return True
