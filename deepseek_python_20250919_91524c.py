...
def list_user_files(user_id):
    ...
    try:
        ...
        # Remove the user prefix from filenames
...
async def play_file_handler(client, message: Message):
    ...
    try:
        ...
        if error_code == '404':
            # File not found, show user their available files
            ...
            if user_files:
                ...
                await status_message.edit_text(
                    ...
                    f"📁 Your available files:\n\n{files_list}\n\n"
                    f"💡 Use exact filename from the list above"
                )
            else:
                await status_message.edit_text(
                    f"❌ File not found: {file_name}\n\n"
                    f"You don't have any files uploaded yet.\n"
                    f"Send me a file first, then use /play filename"
                )
        else:
        ...
        if error_code == '404':
            ...
            if user_files:
                ...
                await status_message.edit_text(
                    f"❌ File not found: {file_name}\n\n"
                    f"You don't have any files uploaded yet.\n"
                    f"Send me a file first, then use /download filename"
                )
        else:
            await status_message.edit_text(f"❌ S3 Error: {error_code} - {str(e)}")
    except Exception as e:
...
async def list_files(client, message: Message):
    try:
        ...
        files_list = "\n".join([f"• {file}" for file in user_files[:15]])
        
        if len(user_files) > 15:
            files_list += f"\n\n...and {len(user_files) - 15} more files"
        
        await message.reply_text(f"📁 Your files ({len(user_files)} total):\n\n{files_list}\n\n💡 Use /play or /download with the exact filename")
...
async def search_files(client, message: Message):
    if len(message.command) < 2:
        await message.reply_text("Usage: /search <filename_part>")
        return
    ...
    try:
        ...
        matching_files = [file for file in user_files if search_term in file.lower()]
        
        if not matching_files:
            await message.reply_text(f"🔍 No files found containing: '{search_term}'")
            return
        
        files_list = "\n".join([f"• {file}" for file in matching_files[:10]])
        
        if len(matching_files) > 10:
            files_list += f"\n\n...and {len(matching_files) - 10} more matches"
        
        await message.reply_text(f"🔍 Found {len(matching_files)} files containing '{search_term}':\n\n{files_list}")
...
if __name__ == "__main__":
    ...
    # Start HTTP server in a separate thread for Render compatibility
